...
void RayTracer::Run()
{
    ...
    for (int r = 0; r < static_cast<int>(currentResolution.y); ++r) {
        for (int c = 0; c < static_cast<int>(currentResolution.x); ++c) {
            ...
            if ((r < currentResolution.y / 8.f) && (c % 10 == 0)) {
                std::cout << r / currentResolution.y * 800.f + c / currentResolution.x / currentResolution.y * 800.f << "% finished..." << std::endl;
            }
        }
//        if (r < currentResolution.y / 8.f) {
//            std::cout << r / currentResolution.y * 800.f << "% finished..." << std::endl;
//        }
    }
